...
function firstId(body: unknown): string {
  ...
  const id =
    typeof first === 'object' && first !== null
      ...
      : undefined;
  if (typeof id !== 'string')
    throw new Error('Course list does not contain an id');
  return id;
}
...
async function smoke(): Promise<void> {
  for (const resource of ['services', 'projects', 'courses', 'posts']) {
    ...
    assertStatus(list.status, 200, `GET /${resource}`);
    const validSlug =
      resource === 'services'
        ? 'tu-van-bim'
        : resource === 'projects'
          ...
          : resource === 'courses'
            ? 'bim-foundation'
            : 'trien-khai-du-an-trong-diem-2026';
    assertStatus(
      (await request(`/${resource}/${validSlug}`)).status,
      200,
      `GET /${resource}/:slug`,
    );
    assertStatus(
      (await request(`/${resource}/slug-khong-ton-tai`)).status,
      404,
  ...
  assertStatus(
    (
      await request('/course-registrations', {
        method: 'POST',
        body: JSON.stringify({
          courseId: firstId(courses.body),
          name: 'BIM4C Smoke Test',
  ...
  assertStatus(
    (
      await request('/newsletter/subscriptions', {
        method: 'POST',
  ...
  assertStatus(
    ...
    200,
    'POST /newsletter/subscriptions duplicate',
  );
