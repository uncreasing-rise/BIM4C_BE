...
export class PageQueryDto {
  ...
  @IsOptional() @Transform(trimmed) @IsString() @MaxLength(120) category?: string;
  @IsOptional() @IsIn(['asc', 'desc']) sortOrder: 'asc' | 'desc' = 'desc';
  @IsOptional() @IsIn(['publishedAt', 'createdAt', 'title', 'sortOrder']) sortBy: 'publishedAt' | 'createdAt' | 'title' | 'sortOrder' = 'sortOrder';
}
...
export function pageResponse<T>(data: T[], total: number, page: number, limit: number): PageResponse<T> { return { data, meta: { page, limit, total, totalPages: Math.ceil(total / limit) } }; }
