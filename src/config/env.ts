...
const schema = z
  ...
  .superRefine((env, context) => {
    ...
    if (!publicHttps(env.FRONTEND_URL))
      issue('FRONTEND_URL', 'Production frontend origin must be public HTTPS');
    for (const origin of (env.CORS_ORIGINS ?? env.FRONTEND_URL)
      .split(',')
      .map((value) => value.trim())) {
      if (!publicHttps(origin))
        issue(
          'CORS_ORIGINS',
          'Every production CORS origin must be public HTTPS',
        );
    }
    if (env.REVALIDATION_URL && !publicHttps(env.REVALIDATION_URL))
      issue(
        'REVALIDATION_URL',
        'Production revalidation URL must be public HTTPS',
      );
    if (!env.REVALIDATION_SECRET)
      issue('REVALIDATION_SECRET', 'Required in production');
    if (env.MEDIA_STORAGE_DRIVER !== 'supabase')
      issue(
        'MEDIA_STORAGE_DRIVER',
        'Production media storage must use Supabase',
      );
    if (!env.SUPABASE_URL) issue('SUPABASE_URL', 'Required in production');
    if (!env.SUPABASE_SERVICE_ROLE_KEY)
      issue('SUPABASE_SERVICE_ROLE_KEY', 'Required in production');
    if (!env.SUPABASE_MEDIA_BUCKET)
      issue('SUPABASE_MEDIA_BUCKET', 'Required in production');
    if (!publicHttps(env.PUBLIC_API_URL))
      issue('PUBLIC_API_URL', 'Production public API URL must be public HTTPS');
    const databaseHost = new URL(env.DATABASE_URL).hostname;
    if (
      ['localhost', '127.0.0.1', '::1'].includes(databaseHost) ||
      /^192\.168\./.test(databaseHost)
    )
      issue(
        'DATABASE_URL',
        'Production database must not use a loopback or LAN host',
      );
    if (env.TEMPORARY_ADMIN_AUTH)
      issue('TEMPORARY_ADMIN_AUTH', 'Must be false in production');
    if (env.ADMIN_BOOTSTRAP_RESET_PASSWORD)
      issue('ADMIN_BOOTSTRAP_RESET_PASSWORD', 'Must be false in production');
  });
