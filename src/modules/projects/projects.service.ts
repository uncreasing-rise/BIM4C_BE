...
export interface ProjectResponse extends ContentResponse { category: { id: string; name: string; slug: string }; location: string; year: number; investor: string | null; expectedCompletion: string | null; scale: string | null; contractPackage: string | null; status: string; gallery: { id: string; url: string; alt: string; caption: string | null; sortOrder: number }[] }
...
export class ProjectsService {
  ...
  private map(row: Prisma.ProjectGetPayload<{ include: { category: true; images: true } }>): ProjectResponse { return { ...mapContent(row), category: row.category, location: row.location, year: row.year, investor: row.investor, expectedCompletion: row.expectedCompletion, scale: row.scale, contractPackage: row.contractPackage, status: publicStatus(row.status), gallery: row.images }; }
